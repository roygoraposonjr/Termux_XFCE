...
xfce_packages=('xfce4' 'xfce4-goodies' 'xfce4-pulseaudio-plugin' 'firefox' 'starship' 'termux-x11-nightly' 'fastfetch' 'papirus-icon-theme' 'eza' 'bat')
...
env DISPLAY=:0 MESA_VK_WSI_DEBUG=sw LIBGL_KOPPER_DRI2=1 MESA_LOADER_DRIVER_OVERRIDE=zink dbus-launch --exit-with-session xfce4-session & > /dev/null 2>&1
...
#!/bin/bash
varname=$(basename $PREFIX/var/lib/proot-distro/installed-rootfs/ubuntu/home/*)
pd login ubuntu --user $varname --shared-tmp -- env DISPLAY=:0 MESA_VK_WSI_DEBUG=sw LIBGL_KOPPER_DRI2=1 MESA_LOADER_DRIVER_OVERRIDE=zink $@
...
#!/bin/bash
varname=$(basename $PREFIX/var/lib/proot-distro/installed-rootfs/ubuntu/home/*)
pd login ubuntu --user $varname --shared-tmp -- env DISPLAY=:0 MESA_VK_WSI_DEBUG=sw LIBGL_KOPPER_DRI2=1 MESA_LOADER_DRIVER_OVERRIDE=zink GALLIUM_HUD=fps vblank_mode=0 $@
